        ...
        <div x-data="{ tab: 'info' }">
            ...
            <div x-show="tab === 'team'">
                <div class="p-4 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
                    <h2 class="mb-3 font-semibold">Team Members</h2>
                    ...
                    <ul class="mt-4 space-y-2">
                        @foreach($proposal->teamMembers as $member)
                            <li class="flex items-center justify-between pb-1 border-b border-gray-100 dark:border-gray-700">
                                {{ $member->user->name }}
                                <form action="{{ route('be.team-members.destroy', $member->id) }}" method="POST" onsubmit="return confirm('Remove member?')">
                                    @csrf @method('DELETE')
                    ...
                    <ul class="mt-4 space-y-2">
                        @foreach($proposal->attachments as $attachment)
                            <li class="flex items-center justify-between pb-1 border-b border-gray-100 dark:border-gray-700">
                                ...
                                <form action="{{ route('be.attachments.destroy', $attachment->id) }}" method="POST" onsubmit="return confirm('Delete attachment?')">
                                    @csrf @method('DELETE')
